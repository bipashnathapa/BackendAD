...
{rows}
            ...
            {
                if (!await email.SendAsync(recipient.Email!, "[VMS] Low stock alert", html))
                {
                    _log.LogWarning("Low stock alert was not delivered to {Email}", recipient.Email);
                }
            }
        }
...
<p>Thank you,<br/>Vehicle Management</p>";
            ...
            if (!await email.SendAsync(to, $"[VMS] Payment reminder — {inv.InvoiceNumber}", body))
            {
                _log.LogWarning("Payment reminder was not delivered for invoice {InvoiceNumber}", inv.InvoiceNumber);
                continue;
            }
