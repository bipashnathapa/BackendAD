...
public class StaffService : IStaffService
{
    ...
    public async Task<StaffCustomerHistoryDto?> GetCustomerHistoryAsync(int customerId)
    {
        var customer = await _context.Customers
            .Include(c => c.User)
            .Include(c => c.Vehicles)
        ...
        var invoices = await _context.SaleInvoices
            ...
            .Select(i => new StaffCustomerHistoryInvoiceDto
            {
        ...
        return new StaffCustomerHistoryDto
        {
            CustomerId = customer.CustomerID,
            UserId = customer.UserID,
            ...
            PhoneNumber = customer.User?.PhoneNumber,
            Address = customer.User?.Address,
            Vehicles = (customer.Vehicles ?? new List<VehicleInfo>())
                .Select(v => new StaffCustomerHistoryVehicleDto
                {
                    VehicleId = v.VehicleID,
                    VehicleNo = v.VehicleNo,
                    Brand = v.Brand,
